...
export default function MathPlugin(): React.ReactElement | null {
  ...
  useEffect(() => {
    const handleSelectionChange = () => {
      ...
      mathfields.forEach((mathfield) => {
        const startContainerBits =
          range.startContainer.compareDocumentPosition(mathfield)
        const endContainerBits =
          range.endContainer.compareDocumentPosition(mathfield)
        const startMask =
  ...
  useEffect(() => {
    const navigation = (
      window as Window & { navigation?: { addEventListener: (type: string, handler: (e: { navigationType: string; preventDefault(): void }) => void) => void; removeEventListener: (type: string, handler: (e: { navigationType: string; preventDefault(): void }) => void) => void } }
    ).navigation
