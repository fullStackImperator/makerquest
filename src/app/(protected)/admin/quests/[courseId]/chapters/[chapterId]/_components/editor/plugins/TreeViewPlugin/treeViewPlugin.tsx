...
export function TreeView({
  ...
  void timeTravelButtonClassName
  void timeTravelPanelSliderClassName
  void timeTravelPanelButtonClassName
  void timeTravelPanelClassName
  ...
  useEffect(() => {
    ...
    const next = generateContent(editor, commandsLog, showExportDOM)
    const t = setTimeout(() => setContent(next), 0)
    return () => clearTimeout(t)
  }, [commandsLog, editor, showLimited, showExportDOM, isLimited])
  ...
  useEffect(() => {
    if (!timeTravelEnabled) {
      const totalEditorStates = timeStampedEditorStates.length
      const t = setTimeout(() => setSliderValue(totalEditorStates), 0)
      return () => clearTimeout(t)
    }
  ...
  return (
    <div
      className={`relative print:hidden ${viewClassName}`}
    >
...
function useLexicalCommandsLog(
  ...
  useEffect(() => {
    ...
    for (const [, command] of Object.entries(editor._commands)) {
      unregisterCommandListeners.add(
        editor.registerCommand(
          command,
          (payload) => {
